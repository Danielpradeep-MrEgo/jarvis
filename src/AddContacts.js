...
import jarvistext from "./jarvistext.mp4";
...
function AddContacts() {
	...
	// console.log(userId);
	useEffect(() => {
		db.collection("users").doc(user.phoneNumber).collection("profiles");
		// .doc(`${cont}`)
	});
	...
	recognition.onresult = function (event) {
		...
		if (transcript === "add contact") {
			submitContact();
		}
		if (transcript.includes("add contact")) {
			submitContact();
		}
		if (transcript.includes("clear phone")) {
			setCont("");
		}
		if (transcript.includes("clear name")) {
			setPassword("");
		}
		if (transcript.includes("code India")) {
			setValue(91);
		}
		if (transcript.includes("India")) {
			setValue(91);
		}
		if (transcript === "code India") {
			setValue(91);
		}
		if (transcript === "India") {
			setValue(91);
		}
		if (transcript.includes("set number")) {
			// console.log("set con done");
			var num = transcript.replace(/\D/g, "");
			// console.log(num, "num");
			setCont(num);
		}
		...
		if (transcript.includes("set name")) {
			...
			let match = sli.findIndex(
				(item) => item.toLocaleLowerCase().indexOf("yes") > -1
			);
			// console.log(match);
			...
			var add = sli.slice(match + 1, ma - 1);
			// console.log(add, "add");
			setPassword(add);
		}
		...
		// console.log(transcript.includes("add contact"));
		setTrans(transcript);
	};
	///////////////////////////////////////
	...
	function readOutNumber() {
		const speech = new SpeechSynthesisUtterance();
		speech.text = "tell me a number to add sir";
		speech.volume = 1;
		speech.rate = 0.5;
		speech.pitch = 1;
	...
	function readOutShell() {
		const speech = new SpeechSynthesisUtterance();
		speech.text = "shell i add a name to this contact sir";
		speech.volume = 1;
		speech.rate = 0.5;
		speech.pitch = 1;
	...
	function readOutName() {
		const speech = new SpeechSynthesisUtterance();
		speech.text = "tell me a name to add sir";
		speech.volume = 1;
		speech.rate = 0.5;
		speech.pitch = 1;
	...
	return (
		<div className="addContacts__main">
			<div className="addContacts__ani">
				<div>{/* <h1>Jarvis</h1> */}</div>
				<div className="addContacts">
					<div className="addContacts__form">
						<form onSubmit={submit}>
							{/* <h1>Jarvis</h1> */}
							<video
								className="addContacts__video"
								src={jarvistext}
								// autoplay
								autoPlay={true}
								loop
							...
							<div className="button__jarvis">
								<Button onClick={jarvis} style={{ color: "white" }}>
									jarvis
								</Button>
							</div>
							{/* <Button onClick={stop} style={{ color: "white" }}>
								stop
							</Button> */}
						</form>
					</div>
